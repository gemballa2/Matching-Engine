...
public:
	...
	void Add_Order_to_Book(Order ord, bool checked) {
		...
		if ((side == BUY) && (!matched)) {
			...
			if (Limit_Order_Book.first[0].getPrice() == 0) {
				...
				BOOST_LOG_TRIVIAL(info) << "Matching Engine:  inserting BUY LIMIT order into empty BIDS order book - price " << price << std::endl;
			...
			if (!matched) {
				...
				if (Limit_Order_Book.second[0].getPrice() == 0) {								// No orders inserted into LIMIT book yet
					...
					BOOST_LOG_TRIVIAL(info) << "Matching Engine:  inserting SELL LIMIT order into empty ASKS order book " << price << " with ID " << ordID << std::endl;
	...
	void Remove_From_Market_Order_Vec(int side, int count) {
		...
		if (side == BUY) {
			...
			for (int i = 0; i < count; i++) {
				...
				std::cout << "Market_Orders_Buys_Vec size  " << size(Market_Orders_Buys_Vec) << std::endl;
			...
			for (int i = 0; i < count; i++) {
				...
				std::cout << "Market_Orders_Sells_Vec size  " << size(Market_Orders_Sells_Vec) << std::endl;
			}
	...
	void Purge_Limit_Order_Book() {
		...
		BOOST_LOG_TRIVIAL(info) << "Purge_Limit_Order_Book Complete "  << std::endl;
			...
			if (Orders::ordersReady) {
				...
				while (!Orders::orderBuffer.empty()) {
					...
					if (type == 99) {										// Special flush message
						for (auto& sym : ISINs) {
							BOOST_LOG_TRIVIAL(info) << "Matching Engine: type " << type << std::endl;
							Complete_Order_Book[sym].Purge_Limit_Order_Book();
						}
						continue;
					}
...
private:
	...
	std::array<std::string, 5> ISINs{ "GB0007980591"s , "GB00B03MLX29"s, "DE0005190003"s, "IBM"s, "AAPL"s };		//"ES0178430E18"s, "FR0000131104"s };
